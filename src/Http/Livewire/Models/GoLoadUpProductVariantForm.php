...
use Astrogoat\Shopify\Models\Collection;
...
class GoLoadUpProductVariantForm extends Form
{
    ...
    public function mount($goLoadUpProduct = null)
    {
        ...
        $this->selectedOptionsOne = $this->model->products->reject(function ($product) { return $product->pivot->product_option != 1; });
        $this->selectedOptionsOneIds = $this->selectedOptionsOne->map(fn ($product) => $product->id)->toArray();

        $this->selectedOptionsTwo = $this->model->products->reject(function ($product) { return $product->pivot->product_option != 2; });
    ...
    public function setErrorLabel()
    {
        ...
        $variant = $variants->reject(function($item) { return $item->id != $this->model->parent_product_variant_id; });
    ...
    public function insertRecords(SupportCollection $selectedOptions, int $productOption)
    {
        foreach ($selectedOptions as $key => $product) {
            $existingRelations = $this->model->products->reject(function ($product) use ($productOption) { return $product->pivot->product_option != $productOption; });

            $currentMatchFromDB = $existingRelations
                ->reject(function ($itemFromDB) use ($product) { return $itemFromDB->pivot->product_id != $product->id; })
                ->reject(function ($itemFromDB) use ($product, $productOption) { return $itemFromDB->pivot->product_option != $productOption; });
    ...
    public function deleteRecords(SupportCollection $selectedOptions, int $productOption)
    {
        ...
        foreach ($existingRelations as $key => $product) {
            $matchFromSelectedOptions = $selectedOptions
                ->reject(function ($item) use ($productOption, $product) { return $product->pivot->product_id != $item->id; });
